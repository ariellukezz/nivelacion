...
public function registrarIngreso(Request $request)
{
    ...
    $evento = Evento::where('id', $validated['evento_id'])
        ->where('estado', 'activo')
        ->first();
    ...
    if (!$estudiante) {
        return response()->json([
            'message' => 'Estudiante no encontrado o no registrado en el evento.'
        ], 404);
    }
    ...
    if (!$estudiante->firma_ingreso) {
        ...
        $mensaje = 'El estudiante ya registró su ingreso.';
    }
...
{
    $evento = Evento::where('id', $evento_id)
        ->where('estado', 'activo')
        ->first();
    ...
    public function ejecutarSorteo(Request $request)
{
    ...
    $participantes = $query
        ->inRandomOrder()
        ->limit($request->numero_ganadores)
        ->select(
            'e.codigo_est',
            'e.dni',
            'e.nombres',
    ...
    if ($participantes->isEmpty()) {
        return response()->json([
            'message' => 'No existen participantes con asistencia registrada para este evento.'
        ], 404);
    }
